...
void tcp_client_init(void)
{
	...
	tcp_connect(tpcb, &destIPADDR, 1100, tcp_client_connected);
}
...
static err_t tcp_client_connected(void *arg, struct tcp_pcb *newpcb, err_t err)
{
  ...
  if (es != NULL)
  {
    ...
    tcp_write(newpcb, message, strlen(message), TCP_WRITE_FLAG_COPY);
    tcp_output(newpcb); // Ensure the data is sent immediately
